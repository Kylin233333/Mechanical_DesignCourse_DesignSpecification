...
\subsubsection{轴的强度校核}
比较各截面弯矩及轴径，右支点处（B截面）承受巨大的悬臂弯矩，且轴径较小（$d=55\text{mm}$），故B截面为危险剖面。
...
\begin{equation}
    \sigma_{ca} = \sqrt{\sigma^2 + 4\times (\alpha \tau)^2} =
    \sqrt{67.36^2 + 4\times (0.6 \times 16.27)^2} = 70.13 \text{ MPa}
\end{equation}
查表得40Cr(调质)处理，
则轴的许用弯曲应力$[\sigma_{-1b}]=70 \text{ MPa}$，
$\sigma_{ca} \approx [\sigma_{-1b}]$（误差仅0.2\%），考虑到许用应力通常具有安全裕度，故强度满足要求。